...
{
  // Video file path
  std::string video_path = "drone_video_sd_640_338_24fps.mp4";
  ...
  cv::VideoWriter output_video("output_orb_features.mp4", fourcc, fps, cv::Size(width, height));
  ...
  while (true) {
    ...
    // Display frame
    cv::imshow("ORB Features", frame_with_features);
  ...
  std::cout << "Output video saved as: output_orb_features.mp4" << std::endl;
